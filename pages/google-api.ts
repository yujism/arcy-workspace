...
const safeId = (id: unknown) => {if(typeof id !== 'string' || !/^[\w-]{1,150}$/.test(id)) throw new Error('ID Google tidak valid.'); return id;};
...
export async function pagesApi(path: string, body?: any, method = 'POST') {
  ...
  if (body?.action === 'import') {
    ...
    if(meta.mimeType !== 'application/vnd.google-apps.document') throw new Error('Pilih dokumen Google Docs.');
    const content = await (await session.request(`${drive}/${id}/export?mimeType=text%2Fplain`)).text();
    if(content.length > 100000) throw new Error('Dokumen terlalu panjang. Pilih dokumen di bawah 100.000 karakter.');
  ...
  if (body?.action === 'sheets') {
    ...
    if(!range || range.length > 200) throw new Error('Isi range A1 yang valid, maksimal 200 karakter.');
    ...
    if(content.length > 100000) throw new Error('Range terlalu besar. Pilih range yang lebih kecil.');
    const item:RecordItem = {id:'google:sheet:'+await digest(session.owner+':'+id+':'+range), kind:'note', title:range, body:content || '(Range kosong)', updated:now, meta:{provider:'google', googleAccount:session.owner, source:'Google Sheets', tag:'Sheets', url:`https://docs.google.com/spreadsheets/d/${id}/edit`, range}};
    await saveGoogleRecords([item], session.assertCurrent); return {item};
  }
  if (body?.action === 'calendar') {
    if(syncing) throw new Error('Sinkronisasi Calendar masih berjalan.');
    ...
    try {
      ...
      for(let page=0; page<20; page++) {
        ...
        for(const event of result.items || []) {
          ...
          items.push({id:'google:event:'+await digest(session.owner+':'+id), kind:'event', title:event.summary || '(Tanpa judul)', body:event.description || '', updated:now, meta:{provider:'google', googleAccount:session.owner, source:'Google Calendar', googleId:id, url:'https://calendar.google.com/calendar/u/0/r', start:event.start.dateTime || event.start.date, end:event.end.dateTime || event.end.date, location:event.location || ''}});
        }
      ...
      throw new Error('Calendar terlalu besar untuk satu impor. Snapshot sebelumnya tetap ada.');
  ...
  if (body?.action === 'publish-event') {
    ...
    if(!item) throw new Error('Agenda lokal tidak ditemukan.');
    if(!Number.isFinite(Date.parse(item.meta.start)) || !Number.isFinite(Date.parse(item.meta.end)) || Date.parse(item.meta.end) <= Date.parse(item.meta.start)) throw new Error('Waktu agenda tidak valid.');
    ...
    if(response.status === 409) {
      ...
      if(existing.status === 'cancelled' || existing.extendedProperties?.private?.arcyId !== item.id) throw new Error('Agenda ini pernah dikirim atau dihapus di Google. Buat agenda baru untuk mengirim lagi.');
    }
    session.assertCurrent();
    return {ok:true};
  }
  throw new Error('Aksi Google tidak dikenali.');
}
